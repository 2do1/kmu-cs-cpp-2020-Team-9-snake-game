...
void GameScreen(){
  ...
  init_pair(2, COLOR_GREEN,COLOR_BLACK); //fontcolor & game, score window , snake background color
  init_pair(3, COLOR_BLACK,COLOR_WHITE);
  win2 = newwin(13,40,15,60);
  wbkgd(win2,COLOR_PAIR(1)); //score_board color
  wattron(win2,COLOR_PAIR(1)); // score board color
  ;
  ...
  mvwprintw(win2,5,25,"%d",Growth_Item);          // 나중에 body_len 말고 그에 맞는 변수 넣으면 된다.

  mvwprintw(win2,7,10,"Poison-Item:     ");
  mvwprintw(win2,7,25,"%d",Poison_Item);
  ...
  wbkgd(win3,COLOR_PAIR(1));// mission board color
  wattron(win3,COLOR_PAIR(1));// mission board color
...
void reset(){
  ...
  wbkgd(win1,COLOR_PAIR(2));// game board color
  wattron(win1,COLOR_PAIR(3)); // game ttle, snake color
...
void printPause(){
  ...
  WINDOW *w;
   ...
   //int meet=0;
   //bool select_list[3]={false,false,false};
   char item[20];
   ...
   while(( ch = wgetch(w)) != 'q'){
           ...
           /*select_key=getch();
           if((list[1]) && (select_key=='\n')){//clear();
             delwin( w );
             endwin(); exit(0);}  // q 버튼 누르면 터미널로 돌아감.
            else if((meet==1) &&(select_key=='\n')){
               delwin(w);
               startScreen();
               reset();
               endwin();
